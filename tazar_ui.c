...
int main(void) {
    ...
    // There is still a big problem with the window resizing on web. None of the click positions
    // work after that, so there's something that has to happen there with a resize event or
    // something else.
    ...
    while (!WindowShouldClose()) {
        ...
        if (ui_state == UI_STATE_CONFIGURING) {
            ...
            // @todo: Process inputs and handle stuff before writing most of this.
...
//            if (mouse_in_board) {
//                V2 dpos = v2_from_cpos(mouse_cpos);
//                Vector2 screen_pos = (Vector2){game_center.x + (horizontal_offset * (float)dpos.x),
//                                               game_center.y + vertical_offset * (float)dpos.y};
//                DrawPolyLines(screen_pos, 6, hex_radius, 90, PINK);
//            }
        }
